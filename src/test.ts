...
const runQOITestImages = async () => {
	for(const {name, channels} of testImages) {
		await lib.fetchToCanvas(`${assetBase}/${name}.png`, canvas);
		...
		assert(`QOI.encode ${source}`,
			await lib.sha256Bytes(expected),
			await lib.sha256Bytes(encoded));

		const decoded = QOI.decode(encoded);
		assert(`QOI.decode ${source}`,
			await lib.sha256Bytes(rgba),
			await lib.sha256Bytes(decoded.data));
	}
}

const runTestAssets = async () => {
	const source = `${assetBase}/wikipedia_008.png`;
	await lib.fetchToCanvas(source, canvas);
	const rgba = lib.readRGBA(canvas.getContext("2d")!);

	lib.log("<hr>", logElement);
